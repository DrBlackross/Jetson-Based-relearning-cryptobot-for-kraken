...
LIVE_TRADING = False  # ⚠️ SET TO TRUE FOR REAL TRADING - USE AT YOUR OWN RISK!
...
EXCHANGE_FEES = {
    'maker': 0.001,  # 0.1%
    'taker': 0.002,  # 0.2%
    'default': 0.002  # Use taker fees for aggressive strategy
}

# 🔥 AGGRESSIVE TRADING SETTINGS
TRADE_PERCENTAGE = 0.5  # Increased from 5% to 15% - more aggressive position sizing
...
# Test multiple ATR multipliers for volatility adaptation
ATR_MULTIPLIERS_TO_TEST = [0.05, 0.1, 0.15, 0.2, 0.25]  # More aggressive range

# Test multiple learning rates to find optimal one
LEARNING_RATES_TO_TEST = [1e-6, 1e-5, 1e-4, 5e-4]  # Learning rate search space

RETRAIN_INTERVAL_HOURS = 6  # Retrain every 6 hours

# --- AGGRESSIVE TECHNICAL INDICATOR CONFIGURATION ---
INDICATOR_CONFIG = {
    'BASE_FEATURES': ['open', 'high', 'low', 'close', 'volume'],
    # 🔥 More sensitive RSI for aggressive trading
    'RSI': {'enabled': True, 'length': 3, 'overbought': 75, 'oversold': 25},  # Tighter bands
    'MACD': {'enabled': True, 'fast': 8, 'slow': 21, 'signal': 5},  # Faster settings
    'BBANDS': {'enabled': True, 'length': 5, 'std': 1.5},  # Tighter Bollinger Bands
    'OBV': {'enabled': True},
    'ADX': {'enabled': True, 'length': 10},  # Shorter ADX for faster signals
    'CCI': {'enabled': True, 'length': 10, 'c': 0.015},  # More sensitive CCI
    'ATR': {'enabled': True, 'length': 10},  # Shorter ATR for faster adaptation
    'VWAP': {'enabled': False},
    'NATR': {'enabled': True, 'length': 10},
    'TRIX': {'enabled': True, 'length': 3, 'signal': 5},  # Faster TRIX
    'STOCH': {'enabled': True, 'k': 2, 'd': 3, 'smooth_k': 2},  # Faster Stochastic
    'EMA': {'enabled': True, 'lengths': [5, 10]}  # Shorter EMAs for faster signals
}
...
def dashboard():
    ...
    if trader:
        ...
        profit_loss_pct = (
                                  profit_loss / trader.initial_portfolio_value) * 100 if trader.initial_portfolio_value != 0 else 0
    ...
    return render_template_string(HTML_TEMPLATE,
                                  ...
                                  last_decision_time=trader.last_trade_time.strftime(
                                      '%Y-%m-%d %H:%M:%S') if trader and trader.last_trade_time else 'Never',
                                  ...
                                  last_training_time=app_state['last_training_time'].strftime('%Y-%m-%d %H:%M:%S') if
                                  app_state['last_training_time'] else 'Never',
                                  next_training_time=app_state['next_training_time'].strftime('%Y-%m-%d %H:%M:%S') if
                                  app_state['next_training_time'] else 'Calculating...',
...
# --- Helper Function for Indicator Calculation ---
def calculate_all_indicators(df):
    """Calculates all technical indicators based on INDICATOR_CONFIG using TA-Lib."""
    try:
        ...
        if INDICATOR_CONFIG['CCI']['enabled']:
            ...
            df[f"CCI_{cci_config['length']}_{cci_config['c']}"] = talib.CCI(high, low, close,
                                                                            timeperiod=cci_config['length'])
        ...
        if INDICATOR_CONFIG['TRIX']['enabled']:
            ...
            df[f"TRIX_{trix_config['length']}_{trix_config['signal']}"] = talib.TRIX(close,
                                                                                     timeperiod=trix_config['length'])
            trix_values = df[f"TRIX_{trix_config['length']}_{trix_config['signal']}"].values
            df[f"TRIXs_{trix_config['length']}_{trix_config['signal']}"] = talib.EMA(trix_values,
                                                                                     timeperiod=trix_config['signal'])
...
def apply_trading_fees(amount, price, side, fee_rate=None):
    """Apply fees to trade calculations during training and trading"""
    if fee_rate is None:
    ...
    if side == 'BUY':
        return trade_value + fee_amount  # You pay more when buying
    else:  # SELL
        return trade_value - fee_amount  # You receive less when selling


def calculate_profit_with_fees(entry_price, exit_price, amount, fee_rate=None):
    """Calculate profit including fees for both sides"""
    if fee_rate is None:
        fee_rate = EXCHANGE_FEES['default']
...
class CryptoTrader:
    ...
    def __init__(self, model, public_api, private_api, scaler, device, live_trading, crypto_balance, usdt_balance,
                 trading_writer=None):
        self.model = model
        ...
        try:
            if trade_type == 'buy':
                ...
                if self.usdt_balance >= total_cost:
                    ...
                    print(f"🔥 AGGRESSIVE BUY executed: {amount:.4f} {CRYPTO_NAME} at ${price:.5f}")
                    print(f"   Fees: {fee_amount:.4f} USDT ({EXCHANGE_FEES['default'] * 100}%)")
                    return True
                else:
                    print(
                        f"Insufficient USDT for BUY. Needed {total_cost:.2f} (incl. fees), have {self.usdt_balance:.2f}")
                    return False
                ...
                if self.crypto_balance >= amount:
                    ...
                    print(f"🔥 AGGRESSIVE SELL executed: {amount:.4f} {CRYPTO_NAME} at ${price:.5f}")
                    print(f"   Fees: {fee_amount:.4f} USDT ({EXCHANGE_FEES['default'] * 100}%)")
                    return True
                else:
                    print(
    ...
    def make_decision(self):
        ...
        try:
            ...
            # 🔥 AGGRESSIVE RSI OVERRIDES
            if rsi_value >= INDICATOR_CONFIG['RSI']['overbought']:
                predicted_action = "SELL"
                print(
                    f"🔥 RSI {rsi_value:.2f} >= {INDICATOR_CONFIG['RSI']['overbought']} (Overbought) - FORCING AGGRESSIVE SELL")
            elif rsi_value <= INDICATOR_CONFIG['RSI']['oversold']:
                predicted_action = "BUY"
                print(
                    f"🔥 RSI {rsi_value:.2f} <= {INDICATOR_CONFIG['RSI']['oversold']} (Oversold) - FORCING AGGRESSIVE BUY")
            ...
            if predicted_action == "BUY":
                if latest_close > 0:
                    # Account for fees in available balance
                    max_usdt_to_spend = self.usdt_balance * self.trade_percentage
                    # Adjust for fees - we can only spend what we have after fees
...
def fetch_historical_data(public_api, pair, interval, lookback_days):
    """Fetch historical OHLC data from Kraken for training"""
...
def train_with_double_optimization(df, device, public_api):
    """Optimizes both ATR multipliers AND learning rates - NOW WITH FEE-AWARE TARGETS"""
    ...
    for multiplier in ATR_MULTIPLIERS_TO_TEST:
        ...
        # Create targets with current multiplier - NOW INCLUDING FEE CALCULATIONS
        df_temp = df.copy()
        df_temp['future_price'] = df_temp['close'].shift(-SEQUENCE_LENGTH)

        # Calculate profit thresholds including fees
        df_temp['threshold'] = df_temp[f"ATRr_{INDICATOR_CONFIG['ATR']['length']}"] * multiplier
        df_temp['buy_threshold_with_fees'] = df_temp['close'] + df_temp['threshold'] + (
                    df_temp['close'] * EXCHANGE_FEES['default'])
        df_temp['sell_threshold_with_fees'] = df_temp['close'] - df_temp['threshold'] - (
                    df_temp['close'] * EXCHANGE_FEES['default'])

        df_temp['target'] = 1  # HOLD
        # Only mark as BUY if profit after fees exceeds threshold
        df_temp.loc[df_temp['future_price'] > df_temp['buy_threshold_with_fees'], 'target'] = 2  # BUY
        # Only mark as SELL if profit after fees exceeds threshold
        df_temp.loc[df_temp['future_price'] < df_temp['sell_threshold_with_fees'], 'target'] = 0  # SELL
        ...
        for learning_rate in LEARNING_RATES_TO_TEST:
            ...
            # Train multiple models for this combination
            model_infos = []
            ...
            # Select best model for this combination
...
# ... [rest of the functions remain the same, but I'll continue with the main execution part]
...
def initial_checks():
    """
    Performs initial checks for trading mode, API key configuration,
    and API key validity by making a non-trading API call.
    """
    ...
    if LIVE_TRADING:
        ...
        print(
            f"Paper trading balances initialized: {CRYPTO_NAME}={crypto_balance:.{CRYPTO_DECIMALS}f}, USDT={usdt_balance:.2f}")
...
def trading_loop():
    """Main trading loop that runs separately from Flask"""
    global app_state
    ...
    try:
        ...
        TARGET_LEARNING_RATE = 1e-5  # Default
        ...
        if os.path.exists(BEST_MODEL_PATH) and os.path.exists(BEST_SCALER_PATH):
            try:
                ...
                print(
                    f"Loaded existing best model (ATR Multiplier: {TARGET_ATR_MULTIPLIER}, LR: {TARGET_LEARNING_RATE:.2e})")
            except Exception as e:
        ...
        if needs_retraining:
            ...
            try:
                ...
                app_state['last_training_time'] = datetime.now(timezone.utc)
                app_state['next_training_time'] = app_state['last_training_time'] + timedelta(
        ...
        print(f"\n🎯 Starting AGGRESSIVE TRADING with optimized parameters:")
...
if __name__ == "__main__":
    ...
    # Start the trading loop in the main thread
    trading_loop()
